...
+ (instancetype)baseTableViewCellWithCellHeight:(nullable NSNumber *)cellHeight cellModel_id:(nullable NSString *)cellModel_id detail:(nullable CKJBaseTableViewCellRowBlock)detailSettingBlock didSelectRowBlock:(nullable CKJBaseTableViewCellRowBlock)didSelectRowBlock {
...
- (void)setAttText:(NSAttributedString *)attText {
    ...
    if ([attText isKindOfClass:[NSString class]]) {
        _attText = WDCKJAttributed2((NSString *)attText, [UIColor kjwd_colorWithHexString:@"333333"], @16);
    } else if ([attText isKindOfClass:[NSAttributedString class]]) {
        _attText = attText;
    } else if (attText == nil) {
        _attText = nil;
    }
    self.readOnly_TitleLabel.attributedText = _attText;
}
...
- (void)setupData:(CKJBaseTableViewCellModel *)model section:(NSInteger)section row:(NSInteger)row selectIndexPath:(NSIndexPath *)indexPath tableView:(CKJSimpleTableView *)tableView {
    ...
    model.readOnly_TitleLabel = titleLab;
