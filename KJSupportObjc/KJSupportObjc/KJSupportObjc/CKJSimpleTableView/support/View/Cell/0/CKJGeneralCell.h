...
typedef void(^CKJArrow9Block)(__weak CKJArrow9Model *_a);
...
@property (copy, nonatomic, nullable) CKJArrow9Block click;
...
/*
 ...
 simpleTableView.updateStyle { (style) in
     style.rowHeight = 45
 }
 
 let arr: [[AnyObject]] = [
     [
         CKJGeneralItem.init(image: nil, title: "忘记密码", detail: nil, click: { (temp) in
             print("点击挂号")
         }),
         CKJGeneralItem.init(image: nil, title: "意见反馈", detail: nil, click: { (temp) in
             print("点击收藏")
         }),
         CKJGeneralItem.init(image: nil, title: "用户协议", detail: nil, click: { (temp) in
             print("点击收藏")
         }),
         CKJGeneralItem.init(image: nil, title: "隐私政策", detail: nil, click: { (temp) in
             print("点击收藏")
         }),
         CKJGeneralItem.init(image: nil, title: "手势密码", detail: nil, click: { (temp) in
             print("点击收藏")
         }),
         CKJGeneralItem.init(image: nil, title: "联系我们", detail: nil, click: { [weak self](temp) in
             self?.navigationController?.pushViewController(DMModifyPwdVC(), animated: true)
         }),
         CKJGeneralItem.init(image: nil, title: "注销账号", detail: nil, click: { (temp) in
             print("点击收藏")
         })
     ],
     [
         CKJOneBtnCellModel.oneBtnFillStyle(withCellHeight: nil, title: WDAtt13("退出登录"), radius: nil, bgColor: UIColor.white, detail: nil) { (m, btn) in
             UIAlertController.kjwd_alertTitle("提示提示", message: "确定退出登录吗？", alertAction_Left: "取消", leftBlock: nil, right: "确定", rightBlock: {_ in
                 DMLoginManager.manager.cancelLogin()
             }, detail: nil, presentingVC: self)
             
         } updateConstraint:  { (make, superView) in
             make.edges.equalTo()
         }
     ]
 ]
 
 var sections = [CKJCommonSectionModel]()
 
 for group in arr {
     let section = CKJCommonSectionModel.section { (_sec: CKJCommonSectionModel) in
         for obj in group {
             if let item = obj as? CKJGeneralItem {
                 let title = item.title as? String
                 let click = item.click
                 let m = CKJGeneralCellModel.general(withTitle: title, arrow: CKJArrow9Model(), didSelectRowBlock: click)
                 if let detail = item.detail {
                     detail(m)
                 }
                 _sec.add(m)
             } else if let model = obj as? CKJCommonCellModel {
                 _sec.add(model)
             }
         }
     }
 ...
 simpleTableView.dataArr = sections
}
...
@interface CKJGeneralItem : CKJBaseModel



@property (copy, nonatomic, nullable) id image;

// 可以是 NSString、NSAttributedString 类型
@property (copy, nonatomic) id title;


@property (copy, nonatomic, nullable) CKJGeneralCellModelRowBlock detail;


@property (copy, nonatomic) CKJGeneralCellModelRowBlock click;
...
/// title 和 箭头
+ (instancetype)generalWithTitle:(nullable id)title arrow:(CKJArrow9Model *)arrow didSelectRowBlock:(nullable CKJGeneralCellModelRowBlock)didSelectRowBlock;

/// image title 箭头
+ (instancetype)generalWithImageName:(nullable NSString *)imageName imageSize:(CGSize)imageSize title:(nullable id)title arrow:(BOOL)arrow didSelectRowBlock:(nullable CKJGeneralCellModelRowBlock)didSelectRowBlock;

/// title 右标题
+ (instancetype)generalWithTitle:(nullable id)title likePriceAttText:(nullable id)likePriceAttText setting:(nullable CKJGeneralSetting *)setting didSelectRowBlock:(nullable CKJGeneralCellModelRowBlock)didSelectRowBlock;

/// title 右标题 箭头
+ (instancetype)generalWithTitle:(nullable id)title likePriceAttText:(nullable id)likePriceAttText setting:(CKJGeneralSetting *)setting arrow:(nullable CKJArrow9Model *)arrow didSelectRowBlock:(nullable CKJGeneralCellModelRowBlock)didSelectRowBlock;
