...
//    NSLog(@"进入设置颜色 %@", backgroundColor);
    if (self.simpleStyle.backgroundColor != backgroundColor) {
        self.simpleStyle.backgroundColor = backgroundColor;
//        NSLog(@"设置颜色  %@ ", backgroundColor);
    }
}
...
- (void)kjInit {
    ...
    self.simpleStyle = [[CKJSimpleTableViewStyle alloc] initWithTableView:self];
}
...
- (void)kjwd_reloadSectionBySectionModel:(CKJCommonSectionModel *_Nullable)sectionModel withRowAnimation:(UITableViewRowAnimation)animation {
    if ([sectionModel isKindOfClass:[CKJCommonSectionModel class]]) {
        NSInteger section = [self.dataArr kjwd_indexOfObject:sectionModel].integerValue;
        [self kjwd_reloadSection:section withRowAnimation:animation];
    }
}
                ...
                for (CKJInputExpressionRequiredModel *expModel in m.expressionRequiredArray) {
                    ...
                    if (expModel.required && expModel.requiredExpression) {
                        if (expModel.requiredExpression(m.tfModel.attText.string, m)) {
                            [CKJHUD kjwd_showMessageAutoHide:[NSString stringWithFormat:@"%@", expModel.requiredText] toView:view];
                            return YES;
                        }
                    }
                }
...
- (CKJInputCellModel *)_newtitle:(NSString *_Nullable)title tfText:(NSString *_Nullable)text placeholder:(id _Nullable)placeholder emptyRequirdText:(NSString *_Nullable)emptyRequirdText cellId:(NSString *_Nullable)cellId detail:(nullable CKJInputCellModelRowBlock)detail {
...
- (NSMutableDictionary *)cell_Model_keyValues {
    ...
    NSDictionary *dic = @{
        ...
        NSStringFromClass([CKJDynamicCellModel class]) : @{KJPrefix_cellKEY : NSStringFromClass([CKJDynamicCell class]), KJPrefix_isRegisterNibKEY : @NO},
 ...
 */
+ (instancetype _Nonnull)kjwd_arrayWithResponseDataModels:(NSArray * _Nullable)ResponseDataModels CellModelClass:(Class _Nonnull)CellModelClass callBack:(void(^_Nullable )(__kindof __weak CKJCommonCellModel * _Nonnull currentModel))callBack {
...
+ (instancetype)kjwd_arrayWithResponseDics:(NSArray <NSDictionary *>* _Nullable)responseDics ResponseDataModelClass:(Class)responseDataModelClass CellModelClass:(Class _Nonnull)CellModelClass callBack:(void(^_Nullable )(__kindof __weak CKJCommonCellModel *currentModel, id dataModel))callBack {
    NSMutableArray *result = [NSMutableArray array];
