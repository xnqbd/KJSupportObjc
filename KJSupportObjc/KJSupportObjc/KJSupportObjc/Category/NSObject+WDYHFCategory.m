...
+ (UIViewController *)kjwd_appVisibleVC {
    UIViewController *rootVC = [UIWindow kjwd_appdelegateWindow].rootViewController;
    
    if ([rootVC isKindOfClass:[UITabBarController class]]) {
        UITabBarController *tabbarVC = (UITabBarController *)rootVC;
        UIViewController *selectedViewController = tabbarVC.selectedViewController;
        if ([selectedViewController isKindOfClass:[UINavigationController class]]) {
            UINavigationController *selectNavc = (UINavigationController *)selectedViewController;
            UIViewController *result = selectNavc.visibleViewController;
            return result;
        } else {
            return selectedViewController;
        }
    } else if ([rootVC isKindOfClass:[UINavigationController class]]) {
        UINavigationController *selectNavc = (UINavigationController *)rootVC;
        UIViewController *result = selectNavc.visibleViewController;
        return result;
    }
    return rootVC;
...
/**
 ...
 如果包装的view，高度超过44，那就是44， 如果是小于44(比如10)，那就是多少(比如10)， 如果不设置或者是0，那就是0
 
 */
+ (instancetype)kjwd_itemWithNormalImage:(nullable UIImage *)normalImage wrapperWidth:(CGFloat)wrapperWidth btnSize:(CGSize)btnSize style:(UIBarButtonItemStyle)style detail:(void(^_Nullable)(UIButton *_btn, UIView *_superview))detail click:(void(^)(UIBarButtonItem *i))click {
    
    UIView *wrapper = [[UIView alloc] init];
    // 只能设置size大小，origin始终会是0,0 原点，
    // 如果是leftBarButtonItem， 那么始终会距离左边一定距离
    // 如果是rightBarButtonItem，那么始终会距离右边一定距离
    wrapper.frame = CGRectMake(0, 0, wrapperWidth, 44);
...
//    WDCKJBGColor_Arc4Color(btn);
    ...
    btn.center = wrapper.center;
    
    detail ? detail(btn, wrapper) : nil;
...
- (void)kjwd_addSubview:(UIView *)view {
    if (!WDKJ_IsNullObj(view, [UIView class])) {
        [self addSubview:view];
    }
}
- (void)kjwd_addSubviews:(NSArray <UIView *>*)views {
...
- (void)kjwd_addTapGestureRecognizerHandleBlock:(void(^)(UITapGestureRecognizer *_gestureRecognizer, UIView *_currentView))handleBlock {
    if ([self isKindOfClass:[UIImageView class]] || [self isKindOfClass:[UILabel class]]) {
...
/// 根据颜色生成实心圆
+ (UIImage *)kjwd_getFill_ellipseWithSize:(CGSize)size color:(UIColor *)color {
...
/// 根据颜色生成空心圆
+ (UIImage *)kjwd_getStroke_ellipseWithSize:(CGSize)size color:(UIColor *)color lineWidth:(NSNumber *_Nullable)lineWidth {
    if (WDKJ_IsNullObj(color, [UIColor class])) {
        color = [UIColor blackColor];
    }
    if (WDKJ_IsNull_Num(lineWidth)) {
        lineWidth = @1;
    }
    
    UIGraphicsBeginImageContextWithOptions(size, NO, 0);
    
    CGFloat width = lineWidth.floatValue;
    
    [color set];
    UIBezierPath *path = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(width / 2, width / 2, size.width - width, size.height - width)];
    path.lineWidth = width;
    // 贝塞尔曲线的 stroke 内部会获取上下文, 进行添加到上下文和渲染
    [path stroke];
...
/// 身份证号
- (BOOL)kjwd_validateIdentityCardSuccess {
    BOOL flag;
    if (self.length <= 0) {
        flag = NO;
        return flag;
...
- (NSString *)kjwd_getIp {
    ...
    [[originStr kjwd_searchWithRegularExpression:reg options:NSRegularExpressionCaseInsensitive] enumerateObjectsUsingBlock:^(NSTextCheckingResult * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSRange range = [obj rangeAtIndex:1];
        NSString *temp = [originStr substringWithRange:range];
        [reg_result addObject:temp];
    }];
    NSString *target = reg_result.firstObject;
    if (WDKJ_IsEmpty_Str(target)) {
        target = @"";
    }
    return target;
}
