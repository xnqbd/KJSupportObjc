...
/**
 *  本地读取Json, 返回值可能是NSArray或NSDictionary
 */
 ...
 * 字典转json字符串
 */
-(nullable NSString *)convertToJsonString:(nullable NSDictionary *)dict;
...
+ (nullable NSMutableArray *)readModelArrayFromArray:(NSArray *_Nullable)array modelClass:(Class _Nullable )ModelClass;

/**
 *  通过一个数组形式的json返回 模型数组
 */
+ (nullable NSMutableArray *)readModelArrayFrom_arrayJson:(NSString *_Nullable)arrayJson modelClass:(Class _Nullable )ModelClass;

/**
 *  将自定义对象数组写入沙盒， 路径path可以为空，有默认路径
 */
+ (BOOL)writeCustomClassArray:(NSArray *_Nonnull)customClassArray path:(NSString *_Nullable)path deleteOldData:(BOOL)deleteOldData;
/**
