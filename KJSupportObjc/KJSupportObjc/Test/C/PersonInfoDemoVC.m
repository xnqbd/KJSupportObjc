...
//    CKJGeneralCellModel *model103 = block(@"手机号", @"15522229999");
    ...
    CKJGeneralCellModel *card = [CKJGeneralCellModel modelWithCellHeight:44 cellModel_id:nil detailSettingBlock:^(__kindof CKJGeneralCellModel * _Nonnull m) {
        ...
        m.likePrice8Model = [CKJLikePriceLabel8Model likePriceLabel8ModelWithAttText:WDCKJAttributed2(@"点击查看", [UIColor systemBlueColor], @13) left:0 right:15];
        m.arrow9Model = [CKJArrow9Model arrow9SystemModel];
    } didSelectRowBlock:^(__kindof CKJGeneralCellModel *__weak  _Nonnull m) {
        NSLog(@"点击了就诊卡");
    }];
    
    CKJGeneralCellModel *status = [CKJGeneralCellModel modelWithCellHeight:44 cellModel_id:nil detailSettingBlock:^(__kindof CKJGeneralCellModel * _Nonnull m) {
    ...
    if (NO) { // 大于14岁
        CKJCommonSectionModel *section1 = [CKJCommonSectionModel sectionWithFooterHeight:10 detailSetting:^(__kindof CKJCommonSectionModel * _Nonnull sec) {
            sec.modelArray = @[name, idCardType, idCardNumber, status, sex, birthday, relate, phone];
        }];
        CKJCommonSectionModel *section2 = [CKJCommonSectionModel sectionWithFooterHeight:10 detailSetting:^(__kindof CKJCommonSectionModel * _Nonnull sec) {
            sec.modelArray = @[card];
        }];
        ...
        CKJCommonSectionModel *section3 = [CKJCommonSectionModel sectionWithFooterHeight:10 detailSetting:^(__kindof CKJCommonSectionModel * _Nonnull sec) {
            sec.modelArray = @[jianhuren_name, jianhuren_idCard, status, relate, phone];
        }];
