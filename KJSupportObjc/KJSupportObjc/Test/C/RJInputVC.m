...
- (void)updateUI {
    void(^block)(NSInteger cellModelId, NSString *text) = ^(NSInteger cellModelId, NSString *text) {
        [self.simpleTableView kjwd_filterCellModelForID:cellModelId finishBlock:^(__kindof CKJInputCellModel * _Nonnull m) {
            [m.tfModel _setText:text];
        }];
    };
    
    block(kInput_Name, @"张三");
    block(kInput_Sex, @"男");
    block(kInput_idCardType, @"身份证");
    block(kInput_idCardNumber, @"4139231995072717283");
...
//        CKJInputCellModel *zhengjianType = [self image:@"jzk_card" title:@"证件类型" tfText:@"" placeholder:@"" haveStar:YES cellId:idCardType style:RJInputStyle_Choose didSelectRowBlock:nil];
//        CKJInputCellModel *zhengjianhao = [self image:@"jzk_card" title:@"证件号" tfText:@"" placeholder:@"请输入证件号" haveStar:YES cellId:kInput_idCardNumber style:RJInputStyle_Normal didSelectRowBlock:nil];
...
//        m.tfModel = [CKJTFModel modelWithDetailSettingBlock:^(__kindof CKJTFModel * _Nonnull tfM) {
//            [tfM _setPlaceholder:placeholder];
//            [tfM _setText:text];
//            
//            // 选择
